...
def main():
    ...
    p.add_argument('--clip-model', type=str, default='ViT-B/16',
                   choices=src.open_source.k_diffusion.k_diffusion.evaluation.CLIPFeatureExtractor.available_models(),
    ...
    p.add_argument('--dinov2-model', type=str, default='vitl14',
                   choices=src.open_source.k_diffusion.k_diffusion.evaluation.DINOv2FeatureExtractor.available_models(),
    ...
    config = src.open_source.k_diffusion.k_diffusion.config.load_config(args.config)
    ...
    inner_model = src.open_source.k_diffusion.k_diffusion.config.make_model(config)
    ...
    if accelerator.is_main_process:
        print(f'Parameters: {src.open_source.k_diffusion.k_diffusion.utils.n_params(inner_model):,}')
    ...
    if use_wandb:
        ...
        log_config['parameters'] = src.open_source.k_diffusion.k_diffusion.utils.n_params(inner_model)
    ...
    if sched_config['type'] == 'inverse':
        sched = src.open_source.k_diffusion.k_diffusion.utils.InverseLR(opt,
                                                                        inv_gamma=sched_config['inv_gamma'],
                                                                        power=sched_config['power'],
                                                                        warmup=sched_config['warmup'])
    elif sched_config['type'] == 'exponential':
        sched = src.open_source.k_diffusion.k_diffusion.utils.ExponentialLR(opt,
                                                                            num_steps=sched_config['num_steps'],
                                                                            decay=sched_config['decay'],
                                                                            warmup=sched_config['warmup'])
    elif sched_config['type'] == 'constant':
        sched = src.open_source.k_diffusion.k_diffusion.utils.ConstantLRWithWarmup(opt, warmup=sched_config['warmup'])
    else:
        raise ValueError('Invalid schedule type')

    assert ema_sched_config['type'] == 'inverse'
    ema_sched = src.open_source.k_diffusion.k_diffusion.utils.EMAWarmup(power=ema_sched_config['power'],
    ...
    tf = transforms.Compose([
        ...
        src.open_source.k_diffusion.k_diffusion.augmentation.KarrasAugmentationPipeline(model_config['augment_prob'], disable_all=model_config['augment_prob'] == 0),
    ])

    if dataset_config['type'] == 'imagefolder':
        train_set = src.open_source.k_diffusion.k_diffusion.utils.FolderOfImages(dataset_config['location'], transform=tf)
        ...
        train_set.set_transform(partial(src.open_source.k_diffusion.k_diffusion.utils.hf_datasets_augs_helper, transform=tf, image_key=dataset_config['image_key']))
    ...
    with torch.no_grad(), src.open_source.k_diffusion.k_diffusion.models.flops.flop_counter() as fc:
    ...
    if accelerator.num_processes == 1:
        args.gns = False
    if args.gns:
        gns_stats_hook = src.open_source.k_diffusion.k_diffusion.gns.DDPGradientStatsHook(inner_model)
        gns_stats = src.open_source.k_diffusion.k_diffusion.gns.GradientNoiseScale()
    else:
        gns_stats = None
    sigma_min = model_config['sigma_min']
    sigma_max = model_config['sigma_max']
    sample_density = src.open_source.k_diffusion.k_diffusion.config.make_sample_density(model_config)

    model = src.open_source.k_diffusion.k_diffusion.config.make_denoiser_wrapper(config)(inner_model)
    model_ema = src.open_source.k_diffusion.k_diffusion.config.make_denoiser_wrapper(config)(inner_model_ema)
    ...
    if evaluate_enabled:
        if args.evaluate_with == 'inception':
            extractor = src.open_source.k_diffusion.k_diffusion.evaluation.InceptionV3FeatureExtractor(device=device)
        elif args.evaluate_with == 'clip':
            extractor = src.open_source.k_diffusion.k_diffusion.evaluation.CLIPFeatureExtractor(args.clip_model, device=device)
        elif args.evaluate_with == 'dinov2':
            extractor = src.open_source.k_diffusion.k_diffusion.evaluation.DINOv2FeatureExtractor(args.dinov2_model, device=device)
        else:
            raise ValueError('Invalid evaluation feature extractor')
        train_iter = iter(train_dl)
        if accelerator.is_main_process:
            print('Computing features for reals...')
        reals_features = src.open_source.k_diffusion.k_diffusion.evaluation.compute_features(accelerator, lambda x: next(train_iter)[image_key][1], extractor, args.evaluate_n, args.batch_size)
        if accelerator.is_main_process and not args.evaluate_only:
            metrics_log = src.open_source.k_diffusion.k_diffusion.utils.CSVLogger(f'{args.name}_metrics.csv', ['step', 'time', 'loss', 'fid', 'kid'])
        del train_iter
    ...
    @torch.no_grad()
    @src.open_source.k_diffusion.k_diffusion.utils.eval_mode(model_ema)
    def demo():
        if accelerator.is_main_process:
            tqdm.write('Sampling...')
        ...
        sigmas = src.open_source.k_diffusion.k_diffusion.sampling.get_sigmas_karras(50, sigma_min, sigma_max, rho=7., device=device)
        x_0 = src.open_source.k_diffusion.k_diffusion.sampling.sample_dpmpp_2m_sde(model_fn, x, sigmas, extra_args=extra_args, eta=0.0, solver_type='heun', disable=not accelerator.is_main_process)
        x_0 = accelerator.gather(x_0)[:args.sample_n]
        if accelerator.is_main_process:
            grid = utils.make_grid(x_0, nrow=math.ceil(args.sample_n ** 0.5), padding=0)
            src.open_source.k_diffusion.k_diffusion.utils.to_pil_image(grid).save(filename)
            if use_wandb:
    ...
    def evaluate():
        ...
        def sample_fn(n):
            ...
            if num_classes:
                ...
                model_fn = make_cfg_model_fn(model_ema)
            x_0 = src.open_source.k_diffusion.k_diffusion.sampling.sample_dpmpp_2m_sde(model_fn, x, sigmas, extra_args=extra_args, eta=0.0, solver_type='heun', disable=True)
            return x_0
        fakes_features = src.open_source.k_diffusion.k_diffusion.evaluation.compute_features(accelerator, sample_fn, extractor, args.evaluate_n, args.batch_size)
        if accelerator.is_main_process:
            fid = src.open_source.k_diffusion.k_diffusion.evaluation.fid(fakes_features, reals_features)
            kid = src.open_source.k_diffusion.k_diffusion.evaluation.kid(fakes_features, reals_features)
        ...
        while True:
            for batch in tqdm(train_dl, smoothing=0.1, disable=not accelerator.is_main_process):
                ...
                with accelerator.accumulate(model):
                    ...
                    with src.open_source.k_diffusion.k_diffusion.utils.enable_stratified_accelerate(accelerator, disable=args.gns):
                    ...
                    src.open_source.k_diffusion.k_diffusion.utils.ema_update_dict(ema_stats, {'loss': loss}, ema_decay ** (1 / args.grad_accum_steps))
                    if accelerator.sync_gradients:
                        src.open_source.k_diffusion.k_diffusion.utils.ema_update(model, model_ema, ema_decay)
                        ema_sched.step()
