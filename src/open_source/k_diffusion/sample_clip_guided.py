...
def make_cond_model_fn(model, cond_fn):
    def model_fn(x, sigma, **kwargs):
        with torch.enable_grad():
            ...
            cond_denoised = denoised.detach() + cond_grad * src.open_source.k_diffusion.k_diffusion.utils.append_dims(sigma ** 2, x.ndim)
...
def main():
    ...
    config = src.open_source.k_diffusion.k_diffusion.config.load_config(args.config if args.config else args.checkpoint)
    ...
    inner_model = src.open_source.k_diffusion.k_diffusion.config.make_model(config).eval().requires_grad_(False).to(device)
    inner_model.load_state_dict(safetorch.load_file(args.checkpoint))

    accelerator.print('Parameters:', src.open_source.k_diffusion.k_diffusion.utils.n_params(inner_model))
    ...
    @torch.no_grad()
    @src.open_source.k_diffusion.k_diffusion.utils.eval_mode(model)
    def run():
        if accelerator.is_local_main_process:
            tqdm.write('Sampling...')
        sigmas = src.open_source.k_diffusion.k_diffusion.sampling.get_sigmas_karras(args.steps, sigma_min, sigma_max, rho=7., device=device)
        def sample_fn(n):
            x = torch.randn([n, model_config['input_channels'], size[0], size[1]], device=device) * sigmas[0]
            x_0 = src.open_source.k_diffusion.k_diffusion.sampling.sample_dpmpp_2s_ancestral(model_fn, x, sigmas, eta=1., disable=not accelerator.is_local_main_process)
            return x_0
        x_0 = src.open_source.k_diffusion.k_diffusion.evaluation.compute_features(accelerator, sample_fn, lambda x: x, args.n, args.batch_size)
        if accelerator.is_main_process:
            for i, out in enumerate(x_0):
                filename = f'{args.prefix}_{i:05}.png'
                src.open_source.k_diffusion.k_diffusion.utils.to_pil_image(out).save(filename)
