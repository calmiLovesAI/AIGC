    ...
    if config['type'] == 'image_v1':
        ...
        for depth, width, d_ff, self_attn, dropout in zip(config['depths'], config['widths'], config['d_ffs'], config['self_attns'], config['dropout_rate']):
            if self_attn['type'] == 'global':
                self_attn = src.open_source.k_diffusion.k_diffusion.models.image_transformer_v2.GlobalAttentionSpec(self_attn.get('d_head', 64))
            elif self_attn['type'] == 'neighborhood':
                self_attn = src.open_source.k_diffusion.k_diffusion.models.image_transformer_v2.NeighborhoodAttentionSpec(self_attn.get('d_head', 64), self_attn.get('kernel_size', 7))
            elif self_attn['type'] == 'shifted-window':
                self_attn = src.open_source.k_diffusion.k_diffusion.models.image_transformer_v2.ShiftedWindowAttentionSpec(self_attn.get('d_head', 64), self_attn['window_size'])
            elif self_attn['type'] == 'none':
                self_attn = src.open_source.k_diffusion.k_diffusion.models.image_transformer_v2.NoAttentionSpec()
            else:
                raise ValueError(f'unsupported self attention type {self_attn["type"]}')
            levels.append(
                src.open_source.k_diffusion.k_diffusion.models.image_transformer_v2.LevelSpec(depth, width, d_ff, self_attn, dropout))
        mapping = src.open_source.k_diffusion.k_diffusion.models.image_transformer_v2.MappingSpec(config['mapping_depth'], config['mapping_width'], config['mapping_d_ff'], config['mapping_dropout_rate'])
